...
/*** _printunsigned ***/
/**
 * _printunsigned - Printn unsigned numbers
	...
	int flags, int width, int precision, int size)
{
	...
	num = _convertunsignednum(num, size);
	...
	return (_writeunsigned(0, i, buffer, flags, width, precision, size));
}
...
/*** _printhexadecimal ***/
/**
 * _printhexadecimal - Prints unsigned numbers in hexadecimal notation
 * @types: Lista of arguments
 * @buffer: Buffer array to handle print
 * @flags:  Calculates active flags
 * @width: get width
 ...
 * Return: Number of chars printed
 */
int _printhexaupper(va_list types, char buffer[],
	int flags, int width, int precision, int size)
{
	return (_printhexa(types, "0123456789ABCDEF", buffer,
		flags, 'X', width, precision, size));
